...
def list_queues(session: Session = Depends(get_db), _: object = Depends(require_roles(UserRole.VIEWER, UserRole.OPERATOR, UserRole.ADMIN))) -> list[Queue]:
...
def create_queue(payload: QueueCreate, session: Session = Depends(get_db), _: object = Depends(require_roles(UserRole.ADMIN))) -> Queue:
...
def get_queue(queue_id: int, session: Session = Depends(get_db), _: object = Depends(require_roles(UserRole.VIEWER, UserRole.OPERATOR, UserRole.ADMIN))) -> Queue:
    return get_queue_or_404(session, queue_id)


@router.put("/{queue_id}", response_model=QueueRead, summary="Update a queue")
def update_queue(queue_id: int, payload: QueueUpdate, session: Session = Depends(get_db), _: object = Depends(require_roles(UserRole.ADMIN))) -> Queue:
...
def delete_queue(queue_id: int, session: Session = Depends(get_db), _: object = Depends(require_roles(UserRole.ADMIN))) -> Response:
    queue = get_queue_or_404(session, queue_id)
