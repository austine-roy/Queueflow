...
def create_measurement(queue_id: int, payload: MeasurementCreate, session: Session = Depends(get_db)) -> QueueMeasurement:
    queue = get_queue_or_404(session, queue_id)
    return record_measurement(session, queue, payload, get_settings()).measurement
