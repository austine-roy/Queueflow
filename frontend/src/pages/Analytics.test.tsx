...
const { getQueueAnalytics, getQueuePrediction } = vi.hoisted(() => ({ getQueueAnalytics: vi.fn(), getQueuePrediction: vi.fn() }));
vi.mock("../services/api", () => ({ getQueueAnalytics, getQueuePrediction }));
...
describe("Analytics", () => {
  ...
  it("uses the consolidated backend analytics response", async () => {
    ...
    getQueuePrediction.mockResolvedValue({ queue_id: 1, predicted_count: 6, model: "insufficient_history", measurement_count: 1 });
    render(<Analytics />);
